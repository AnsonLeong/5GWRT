...
#define QLOG_VERSION "1.5.12"  //when release, rename to V1.X
...
static struct arguments *parser_args(int argc, char **argv)
{
    ...
    while (-1 != (opt = getopt(argc, argv, "p:s:n:m:f:D::ciqh")))
    {
        switch (opt)
        {
        case 'p':
...
int drv_is_asr(int idProduct, int idVendor)
{
    ...
    if (strncmp(g_platform_choice, "asr", 3) == 0 && (idVendor == 0x3763 && idProduct == 0x3c93))
    {    return 1;}

    if (strncmp(g_platform_choice, "asr", 3) == 0 && (idVendor == 0x3c93 && idProduct == 0xffff))
        return 1;
...
int drv_is_unisoc(int idProduct, int idVendor)
{
    if (idVendor == 0x2c7c && idProduct == 0x0900)
        ...
        return 1;
    else if (strncmp(g_platform_choice, "unisoc", 6) == 0 && (idVendor == 0x3c93 && idProduct == 0xffff))
        return 1;
...
{
    if (idVendor == 0x2c7c && idProduct == 0x0901)
        return 1;
...
__restart:
    ...
    if (s_usb_device_info[cur_module].idVendor == 0x3763 && s_usb_device_info[cur_module].idProduct == 0x3c93)
    {
        memset(g_platform_choice,0,sizeof(g_platform_choice));
        if (s_usb_device_info[cur_module].bcdDevice == 100)
            strcpy(g_platform_choice, "asr");
        else if (s_usb_device_info[cur_module].bcdDevice == 318)
            strcpy(g_platform_choice, "qualcomm");
        qlog_dbg("The currently selected module platform: %s\n",g_platform_choice);
    }

    if (s_usb_device_info[cur_module].idVendor == 0x3c93 && s_usb_device_info[cur_module].idProduct == 0xffff)
    {
        memset(g_platform_choice,0,sizeof(g_platform_choice));
        if (s_usb_device_info[cur_module].bcdDevice == 404)
            strcpy(g_platform_choice, "unisoc");
        else
            strcpy(g_platform_choice, "asr");
        qlog_dbg("The currently selected module platform: %s\n",g_platform_choice);
    }
