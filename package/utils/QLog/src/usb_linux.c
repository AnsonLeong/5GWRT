...
int ql_find_quectel_modules(void)
{
    ...
    while (NULL != (usb = readdir(usb_dir)))
    {
        ...
        if ((ql_dev.idVendor == 0x05c6 && ql_dev.idProduct == 0x9003) //UC15
            ...
            || (ql_dev.idVendor == 0x3763 && ql_dev.idProduct == 0x3c93)
        ...
        if (ql_dev.idVendor == 0x3763 && ql_dev.idProduct == 0x3c93) //判断是1个模块还是2个模块（高通和asr都有）
        {
            snprintf(devpath, sizeof(devpath), "%.24s/%.16s/bcdDevice", usbpath, usb->d_name);
            ql_dev.bcdDevice = get_value_from_file(devpath, 10);
            memset(g_platform_choice,0,sizeof(g_platform_choice));
            if (ql_dev.bcdDevice == 100)
                strcpy(g_platform_choice, "asr");
            else if (ql_dev.bcdDevice == 318)
                strcpy(g_platform_choice, "qualcomm");
        }

        if (ql_dev.idVendor == 0x3c93 && ql_dev.idProduct == 0xffff) //判断是1个模块还是2个模块（展锐和asr都有）
        {
            snprintf(devpath, sizeof(devpath), "%.24s/%.16s/bcdDevice", usbpath, usb->d_name);
            ql_dev.bcdDevice = get_value_from_file(devpath, 10);
            memset(g_platform_choice,0,sizeof(g_platform_choice));
            if (ql_dev.bcdDevice == 404)
